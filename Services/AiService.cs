...
public class AiService : IAiService
{
    ...
    public async Task<List<MaterialQuestionDto>> GenerateQuestionsAsync(Material material, int count)
    {
        ...
        try
        {
            var filePath = Path.Combine(_env.WebRootPath, material.Url?.TrimStart('/') ?? "");
    ...
    private async Task<string> TranscribeAudioAsync(string filePath)
    {
        ...
        // Whisper limits file size to 25MB. For simplicity, we just send it here.
        var transcriptionOpts = new AudioTranscriptionOptions
        {
            ResponseFormat = AudioTranscriptionFormat.Text
        };
        
        var result = await audioClient.TranscribeAudioAsync(fileStream, Path.GetFileName(filePath), transcriptionOpts);
